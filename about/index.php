<!DOCTYPE html>
<html>
<head>
   <?php include($_SERVER['DOCUMENT_ROOT']."/_layout/head.php"); ?>
</head>
<body>
   <div id="wrapper">
      <?php include($_SERVER['DOCUMENT_ROOT']."/_layout/header.php"); ?>
      <div class="main clearfix">
         <div class="bannerwrap">
            <img class="photobanner" src="">
            <div class="bannertext"></div>
         </div>
         <div class="wrap border-bottom shadow-bottom">
            <span class="pagetitle">ABOUT</span>
            <?php $selected = "home"; include($root."/about/about_topnav_template.php"); ?>
         </div>
         <div class="fullpage-contwrap">
            <div class="cont solo">
               <div class="fillcontainer">
                  <img class="fill" src="/resources/images/society/2GM_F12.png">
         			<div class="contentblock">
            			<div>The Regents Scholar Society at UCLA is a academic and social society created for those who have been awarded the Regents Scholarship, the highest award that can be bestowed upon an incoming undergraduate at UCLA. Its function is to:</div>
            			<ul>
               			<li>Promote academic excellence amongst the recipients of the scholarship</li>
               			<li>Provide community service opportunities for those who wish to take advantage of them</li>
               			<li>Foster interpersonal growth amongst Regents Scholars at UCLA</li>
               			<li>Offer resources and guidance to incoming Regents Scholars</li>
               			<li>Facilitate interaction between Regents Scholars across all University of California campuses</li> 
            			</ul>
            			All current Regents Scholars at UCLA are eligible for membership in the society, and there are no entrance requirements save the receipt of the scholarship.
            		</div>
               </div>
            </div>
         </div>
      </div>
      <?php include($_SERVER['DOCUMENT_ROOT']."/_layout/footer.php"); ?>
   </div>
</body>
</html>